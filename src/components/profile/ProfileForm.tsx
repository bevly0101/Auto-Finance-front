...
const ProfileForm = () => {
    ...
    const fetchUserProfile = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('users')
          .select('nome, username, telefone_whatsapp')
          .eq('id', user.id)
          .single();
  ...
  useEffect(() => {
    const hasProfileChanges = 
      profile.nome !== originalProfile.nome ||
      profile.username !== originalProfile.username ||
      profile.countryCode !== originalProfile.countryCode ||
  ...
  const handleSave = async () => {
    ...
    if (profile.username.length > 50) {
      toast({
        title: "Erro de Validação",
        description: "Nome de usuário deve ter no máximo 50 caracteres.",
        variant: "destructive"
      });
      return;
    }
    ...
    if (!/^[a-zA-Z0-9_.-]+$/.test(profile.username.trim())) {
      toast({
        title: "Erro de Validação",
        description: "Nome de usuário deve conter apenas letras, números, pontos, hífens e sublinhados.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      const sanitizedNome = profile.nome.trim().substring(0, 100);
      const sanitizedUsername = profile.username.trim().substring(0, 50);
      
      // Format phone: country code + number (digits only)
      ...
      toast({
        title: "Erro",
        description: "Erro inesperado ao salvar: " + (error.message || 'Erro desconhecido'),
        variant: "destructive"
      });
    } finally {
      setIsSaving(false);
