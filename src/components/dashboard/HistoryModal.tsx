  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const formattedEntries = (entriesResponse.data || []).map(entry => ({
        ...
        tipo: entry.categoria || 'Sem categoria',
        tipo_especificado: entry.forma_pagamento || null
      }));

      const formattedExpenses = (expensesResponse.data || []).map(expense => ({
        ...expense,
        table_type: 'gastos' as const,
        tipo: expense.categoria || 'Sem categoria',
        tipo_especificado: expense.forma_pagamento || null
      }));
