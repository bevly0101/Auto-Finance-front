...
const UserManagement: React.FC = () => {
  ...
  const fetchUserConsistency = async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase.rpc('get_user_consistency_admin_only');
  ...
  const deleteUserCompletely = async (userId: string) => {
    ...
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(userId.trim())) {
      toast({
        title: "Erro",
        description: "ID do usuário deve ser um UUID válido.",
        variant: "destructive"
      });
      return;
    }

    if (!window.confirm('Tem certeza que deseja excluir este usuário completamente? Esta ação não pode ser desfeita.')) {
      return;
    }
    ...
    try {
      ...
      if (functionError) {
        ...
        if (functionError.message.includes('Access denied')) {
          ...
          toast({
            title: "Erro",
            description: "Erro ao excluir usuário: " + functionError.message,
            variant: "destructive"
          });
        }
        return;
      ...
      toast({
        title: "Erro Inesperado",
        description: "Ocorreu um erro durante a exclusão: " + (error.message || 'Erro desconhecido'),
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
